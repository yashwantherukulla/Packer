...
- Deviations + why:
  - **`PACKER_STORE_ROOT: /data/artifacts`, not the plan's `PACKER_ARTIFACT_DIR`.** The filesystem object store reads `${oc.env:PACKER_STORE_ROOT}` (`conf/store/filesystem.yaml`); the plan's `PACKER_ARTIFACT_DIR` is read by nothing. Mapped the plan's intent (point the store at the mounted `/data/artifacts` volume so the E2E byte-exact cross-check reads real `.pak`s off the host bind) onto the real var. `PACKER_DB_DSN` / `PACKER_REDIS_URL` / `PACKER_RUN_DIR` are exact matches.
  - **Worker `-A packer.workers.celery_app`** (not `packer.workers.app`) — same real-module adaptation as Task 7; applied to both `worker-default` and `worker-gpu` commands.
  - **dev-overlay api command uses `uvicorn packer.api.main:create_app --factory --reload`** — the create_app-factory adaptation from Task 7.
  - **Conftest daemon-guard follow-up.** Creating `docker/compose.yml` flips the Task-2 `compose_stack` fixture out of its `COMPOSE_FILE.exists()` skip and into the self-managed `docker compose up` path — which would ERROR (not skip) with the daemon down here. Added a `_docker_available()` guard (mirrors `tests/integration/sandbox`) so the self-managed path skips cleanly when no daemon is reachable. Unaffected in CI: the nightly job sets `PACKER_E2E_BASE_URL`, taking the externally-managed branch that never calls `compose up`.
- **Verified:** `python -c "import yaml; yaml.safe_load(...)"` → both compose files parse (anchors resolve); **`docker compose -f docker/compose.yml config` → OK** (topology, anchors, services, volumes all validate — the CLI validates without a daemon). `uv run pytest tests/e2e -m e2e` → **3 skipped** (clean skip preserved; no errors). `uv run ruff check --fix` + `ruff format` on `tests/e2e` → clean. **`docker compose up --build` (bringing the live stack online) is DEFERRED to the Task-10 clean-checkout test + the Task-12 nightly workflow — Docker daemon down on this host.**
...
- Deviations + why:
  - **api CMD uses `uvicorn packer.api.main:create_app --factory`, not the plan's `main:app`.** Phase-4 `packer.api.main` exposes a `create_app()` factory with no module-level `app`; the `--factory` flag is the faithful adaptation (no src change).
  - **worker CMD/`-A` targets `packer.workers.celery_app`, not the plan's `packer.workers.app`.** The Celery instance lives in `packer.workers.celery_app:app` (tasks import `from packer.workers.celery_app import app`); `packer.workers.__init__` is empty, so `-A packer.workers.app` would fail to resolve. Adapted to the real module.
  - **`docker/.dockerignore` placement caveat.** Kept at the plan's path. The api/worker builds use `context: ..` (repo root), so Docker's context-root ignore is `../.dockerignore`; a plain `docker/.dockerignore` is only auto-honored by BuildKit as a per-Dockerfile ignore when named `<dockerfile>.dockerignore`. Left at the plan's location (no root file added — no redesign); the exclusions are advisory (build correctness is unaffected, only context size). Exercised in the Task-12 nightly build.
- **Verified:** `uvicorn packer.api.main:create_app`, `packer.api.main.create_app`, and `packer.workers.celery_app:app` confirmed present against Phase-4 source; `alembic/env.py` resolves the DSN via `compose_config().db.dsn` (env-interpolated `PACKER_DB_DSN`), so migrate-on-startup works in-container. `uv run pytest -m "not integration and not e2e"` → 166 passed (no code touched). **Image builds (`docker build -f docker/api.Dockerfile .` etc.) are DEFERRED to the Task-12 nightly workflow — Docker daemon is down on this host.**
...
- Deviations + why:
  - **`_ALLOWED` includes `engine/sandbox/static`, not just `engine/sandbox/adapters`.** The plan asserted the sandbox adapter is the *only* module that spawns OS processes, but `static/bandit_scan.py` and `static/semgrep_scan.py` shell out to the bandit/semgrep CLIs. Those trusted analyzers *read* the extracted file text; they never *execute* it — so they don't violate the "code only runs in the sandbox" invariant. Allowlisting `static` (with a documented rationale) keeps the test green and the invariant honest. (Used the allowlist rather than the plan's `# noqa: host-exec` marker, which would trip ruff's RUF100 "unused noqa" on lines with no real lint violation.) Note: the Docker adapter itself talks to the daemon via the `docker` SDK, so it doesn't even match the banned regex.
  - **Behavioral test hits `POST /models` with JSON, not a file upload.** The real endpoint takes a `ModelCreate {source, format}` body (Phase 4), not a multipart file. Adapted to `json={"source":"evil.bin","format":"pickle"}` → the boundary format gate raises `UnsafeModelError` (code `unsafe_model` → 422, `detail` echoes `pickle`), so the assertion holds against the real contract.
  - **`api_client` fixture is local + skip-guarded.** The E2E harness's `api_client` is scoped to `tests/e2e/`; added a sandbox-local one bound to `PACKER_E2E_BASE_URL` that skips cleanly when unset (Docker down here; exercised in the nightly job).
- **Verified:** `uv run pytest tests/integration/sandbox/test_no_host_exec.py -v` → **1 passed** (grep invariant holds across `src/packer`), **1 skipped** (behavioral, no live API). `uv run ruff check --fix` + `ruff format` on the changed dir → clean. Final gate: `uv run pytest -m "not integration and not e2e"` → **166 passed, 19 deselected**; `uv run pytest -m "integration or e2e"` → **19 skipped** (all Docker-backed suites skip cleanly); `uv run mypy src` → clean (104 files); `uv run lint-imports` → **3 contracts kept**. Live upload-gate execution deferred to the Task-12 nightly job (Docker down).

### `test(sandbox): adversarial ADR-008 containment gate + threat model`
- **Task 5.** Added `docs/THREAT-MODEL.md` (attacker / trust boundary / assets / a controls table mapping each ADR-008 flag to its adversarial test / residual risks). Expanded `tests/integration/sandbox/test_containment.py` from the Phase-3 4-test stub to the full **9-test** adversarial gate covering every ADR-008 control: network blocked+recorded (`--network=none`), read-only root EROFS (`--read-only`), tmpfs-only writable (`--tmpfs /scratch`), non-root uid (`--user`), fork-bomb contained (`--pids-limit`), memory OOM-kill (`--memory`), wall-clock timeout (`timed_out`), host paths unreadable (no bind mounts), and a no-new-privileges smoke. Each escape attempt MUST fail.
- Deviations + why:
  - **Kept the module-level `_docker_available()` skipif; used the real `ExecUnit(data=..., lang=...)` signature.** The plan's Task-5 fixture had no Docker guard (would ERROR, not skip, when the daemon is down) and used a non-existent `ExecUnit(contents=...)` kwarg. Adapted to the actual Phase-3 API (`ExecUnit(filename, data, lang)`, `SandboxResult` fields `stdout`/`exit_code`/`timed_out`/`net_attempts`/`duration_s`) and preserved the clean-skip guard the mandate requires.
  - **No `tests/integration/sandbox/__init__.py`.** Same namespace-package rationale as Task 1; the sandbox test dir already had no `__init__.py` and collects fine.
  - **`exit_code not in (0, None)`** for the read-only-root check (not `!= 0`) so a kill (`exit_code=None`) never masquerades as an EROFS failure.
- **Verified:** `uv run pytest tests/integration/sandbox/test_containment.py -m integration` → **9 skipped** (Docker daemon down — skips cleanly as designed). `uv run ruff check --fix tests/integration/sandbox` + `ruff format` → clean; `uv run mypy src` unaffected. Live containment execution + the `packer-sandbox:latest` build are the hard security gate in the Task-12 nightly job (Docker down here).
